...
bool BatchModeTransformer::AddPass(PhysicalPlanPassType type) {
    LOG(INFO) << "add pass " << PhysicalPlanPassTypeName(type);
...
Status BatchModeTransformer::CreatePhysicalProjectNode(const ProjectType project_type, PhysicalOpNode* depend,
    ...
    switch (project_type) {
        ...
        case kGroupAggregation: {
            ...
            CHECK_TRUE(keys != nullptr, kPlanError, "Can not create group agg with non group keys");
            PhysicalGroupAggrerationNode* agg_op = nullptr;
            CHECK_STATUS(CreateOp<PhysicalGroupAggrerationNode>(&agg_op, depend, column_projects, keys));
            *output = agg_op;
            break;
        }
        ...
        switch (type) {
            ...
            case PhysicalPlanPassType::kPassLeftJoinOptimized: {
                if (catalog_->IndexSupport()) {
                    LOG(INFO) << "start left join optimized";
