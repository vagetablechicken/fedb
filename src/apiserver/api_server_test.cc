...
class APIServerTest : public ::testing::Test {
 public:
    APIServerTest() : mc_(new sdk::MiniCluster(6181)), queue_svc_(new APIServerImpl) {}
    ~APIServerTest() {}
    void SetUp() {
        ...
        ASSERT_TRUE(queue_svc_->Init(option));

        // Http server set up
        ASSERT_TRUE(server_.AddService(queue_svc_.get(), brpc::SERVER_DOESNT_OWN_SERVICE, "/demo/* => demo,") == 0)
 ...
 public:
    ...
    std::unique_ptr<APIServerImpl> queue_svc_;
    brpc::Server server_;
};
...
//    std::string url = "http://127.0.0.1:8010/demo/1/2/3/4";
...
int main(int argc, char* argv[]) {
    ...
    {
        // TODO: test
        ::baidu::common::ThreadPool pool;
        pool.DelayTask(10000, [](){
            LOG(INFO) << "I'm the task";
        });
    }
