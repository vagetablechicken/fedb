...
std::shared_ptr<SQLRequestRow> SQLClusterRouter::GetRequestRow(const std::string& db, const std::string& sql,
                                                               ::hybridse::sdk::Status* status) {
    if (status == nullptr) {
        LOG(WARNING) << "output status is null";
        return {};
    }
    ...
    if (!ok) {
        APPEND_FROM_BASE_AND_WARN(status, vm_status, "fail to explain sql " + sql);
        return {};
    }
...
bool SQLClusterRouter::ExecuteDDL(const std::string& db, const std::string& sql, hybridse::sdk::Status* status) {
    ...
    if (plan_trees.empty() || sql_status.code != 0) {
        APPEND_FROM_BASE_AND_WARN(status, sql_status, "ddl");
        return false;
    }
    ...
    if (!ret.OK()) {
        APPEND_FROM_BASE_AND_WARN(status, ret, "fail to execute sql " + sql);
        return false;
    }
    return true;
}
...
bool SQLClusterRouter::DropTable(const std::string& db, const std::string& table, hybridse::sdk::Status* status) {
    ...
    if (!RefreshCatalog()) {
        SET_STATUS_AND_WARN(status, StatusCode::kCmdError, "Fail to refresh catalog");
        return false;
    }
    ...
    if (tableInfo.has_offline_table_info()) {
        ...
        if (!taskmanager_client_ptr) {
            SET_STATUS_AND_WARN(status, StatusCode::kCmdError, "no TaskManager exist");
            return false;
        }
...
std::shared_ptr<SQLCache> SQLClusterRouter::GetSQLCache(const std::string& db, const std::string& sql,
    ...
    if (!router_cache) {
        ...
        if (cluster_sdk_->GetEngine()->Explain(sql, db, engine_mode, parameter_schema_raw, &explain, &base_status)) {
            ...
            status->code = -1;  // TODO(hw): status code
            return {};
        }
    }
    return router_cache;
...
std::shared_ptr<hybridse::sdk::ResultSet> SQLClusterRouter::ExecuteSQLRequest(const std::string& db,
    ...
    if (!client) {
        SET_STATUS_AND_WARN(status, StatusCode::kCmdError,"not tablet found");
        return {};
    }
    if (!client->Query(db, sql, row->GetRow(), cntl.get(), response.get(), options_->enable_debug)) {
        SET_STATUS_AND_WARN(status, StatusCode::kCmdError,"request server error, msg: " + response->msg());
        return {};
    }
    if (response->code() != ::openmldb::base::kOk) {
        SET_STATUS_AND_WARN(status, StatusCode::kCmdError, "request error, " + response->msg());
        return {};
    }
...
std::shared_ptr<::hybridse::sdk::ResultSet> SQLClusterRouter::ExecuteSQLParameterized(
    ...
    if (!status->IsOK() || !client) {
        CODE_PREPEND_AND_WARN(status, StatusCode::kCmdError, "no tablet available");
        return {};
    }
    auto cntl = std::make_shared<::brpc::Controller>();
    cntl->set_timeout_ms(options_->request_timeout);
...
std::shared_ptr<hybridse::sdk::ResultSet> SQLClusterRouter::ExecuteSQLBatchRequest(
    ...
    hybridse::sdk::Status* status) {
    if (!row_batch || !status) {
        LOG(WARNING) << "input arg is invalid";
        return nullptr;
    }
    auto cntl = std::make_shared<::brpc::Controller>();
    cntl->set_timeout_ms(options_->request_timeout);
    ...
    if (!client) {
        SET_STATUS_AND_WARN(status, StatusCode::kCmdError, "no tablet found");
        return nullptr;
    }
    if (!client->SQLBatchRequestQuery(db, sql, row_batch, cntl.get(), response.get(), options_->enable_debug)) {
        SET_STATUS_AND_WARN(status, StatusCode::kCmdError,"request server error " + response->msg());
        return nullptr;
    }
    if (response->code() != ::openmldb::base::kOk) {
        sSET_STATUS_AND_WARN(status, StatusCode::kCmdError, response->msg());
        return nullptr;
    }
...
bool SQLClusterRouter::ExecuteInsert(const std::string& db, const std::string& sql, ::hybridse::sdk::Status* status) {
    ...
    if (!ret || tablets.empty()) {
        SET_STATUS_AND_WARN(status, StatusCode::kCmdError,"Fail to execute insert statement: fail to get " + table_info->name() + " tablets");
        return false;
    }
    size_t cnt = 0;
...
std::shared_ptr<hybridse::sdk::ResultSet> SQLClusterRouter::HandleSQLCmd(const hybridse::node::CmdPlanNode* cmd_node,
    ...
    if (cmd_node == nullptr || status == nullptr) {
        *status = {StatusCode::kCmdError, "null pointer"};
        return {};
    }
    auto ns_ptr = cluster_sdk_->GetNsClient();
    if (!ns_ptr) {
        *status = {StatusCode::kCmdError, "no nameserver exist"};
        return {};
    }
    bool ret = true;
    std::string msg;
    switch (cmd_node->GetCmdType()) {
        ...
        case hybridse::node::kCmdShowFunctions: {
            ...
            if (!st.OK()) {
                *status = {StatusCode::kCmdError, "show udf function failed"};
                return {};
            }
        ...
        case hybridse::node::kCmdDropIndex: {
            ...
            ret == true ? * status = {} : * status = {StatusCode::kCmdError, msg};
            return {};
        }
...
std::shared_ptr<hybridse::sdk::ResultSet> SQLClusterRouter::ExecuteSQL(
    ...
    hybridse::plan::PlanAPI::CreatePlanTreeFromScript(sql, plan_trees, &node_manager, sql_status);
    if (sql_status.code != 0) {
        *status = {StatusCode::kCmdError, sql_status.msg, sql_status.GetTraces()};
        return {};
    }
    auto ns_ptr = cluster_sdk_->GetNsClient();
    if (!ns_ptr) {
...
std::shared_ptr<hybridse::sdk::ResultSet> SQLClusterRouter::ExecuteOfflineQuery(const std::string& db,
    ...
    if (is_sync_job) {
        ...
        if (job_info.id() <= 0) {
            *status = {StatusCode::kCmdError, "job info id should > 0"};
            return {};
        }
...
hybridse::sdk::Status SQLClusterRouter::HandleLoadDataInfile(
    ...
    if (!st.OK()) {
        return {StatusCode::kCmdError, st.msg};
    }
    ...
    do {
        ...
        if (!ret.IsOK()) {
            return {StatusCode::kCmdError, "line [" + line + "] insert failed, " + ret.msg};
        }
        ++i;
...
bool SQLClusterRouter::ExecuteDelete(std::shared_ptr<SQLDeleteRow> row, hybridse::sdk::Status* status) {
    ...
    auto table_info = cluster_sdk_->GetTableInfo(db, table_name);
    if (!table_info) {
        *status = {StatusCode::kCmdError, "table " + table_name + " in db " + db + " does not exist"};
        return false;
    }
...
hybridse::sdk::Status SQLClusterRouter::HandleDeploy(const std::string& db,
    ...
    if (!cluster_sdk_->GetEngine()->Explain(select_sql, db, hybridse::vm::kMockRequestMode, &explain_output,
                                            &sql_status)) {
        return {StatusCode::kCmdError, sql_status.GetMsg(), sql_status.GetTraces()};
    }
    // pack ProcedureInfo
    ::openmldb::api::ProcedureInfo sp_info;
    sp_info.set_db_name(db);
    ...
    if (!cluster_sdk_->GetEngine()->GetDependentTables(select_sql, db, ::hybridse::vm::kBatchMode, &table_pair,
                                                       sql_status)) {
        return {StatusCode::kCmdError, "get dependent table failed"};
    }
    ...
    if (!status.OK()) {
        return {StatusCode::kCmdError, status.msg};
    }
    return {};
}
