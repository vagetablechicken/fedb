...
class StandaloneConfValidator:
    ...
    def validate(self) -> bool:
        ...
        #     log.warning(
        #         "tablet {} in ns conf and endpoint {} in tablet conf do not match".format(
        #             self.ns_conf_dict["tablet"], self.tablet_conf_dict["endpoint"]
        #         )
        #     )
        #     flag = False
        # if (
        #     "system_table_replica_num" in self.ns_conf_dict
        #     and int(self.ns_conf_dict["system_table_replica_num"]) != 1
        # ):
