...
class StatusChecker:
    ...
    #             logging.warning(f"{component} number is greater than 1")
    ...
    #                 break
    #         if not has_found:
    #             logging.warning(f"{component} endpoint {endpoint} has not startup")
...
class SQLTester:
    ...
    def setup(self):
        ...
        tables = self.conn.execfetch(f"SHOW TABLES")
        assert len(tables) == 1 and tables[0][0] == self.table_name, f"cur tables: {tables}, > 1 table or not {self.table_name}"
