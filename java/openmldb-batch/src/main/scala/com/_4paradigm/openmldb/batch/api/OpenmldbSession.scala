      ...
      tableInfos.map(tableInfo => {
        ...
        if (offlineTableInfo != null) { // offlineTableInfo is always not null
          ...
          try {
            ...
            case e: Exception => {
              logger.warn(s"Fail to register table $dbName.$tableName debug " + ExceptionUtils.getStackTrace(e))
              // logger.warn(s"Fail to register table $dbName.$tableName", e.getStackTrace)
            }
          }
        }
      })
