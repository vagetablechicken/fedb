...
public class BatchCallablePreparedStatementImpl extends CallablePreparedStatement {
    ...
    public SQLResultSet executeQuery() throws SQLException {
        ...
        com._4paradigm.openmldb.ResultSet resultSet = router.CallSQLBatchRequestProcedure(db, spName, currentRowBatch,
                status);
    ...
    public QueryFuture executeQueryAsync(long timeOut, TimeUnit unit) throws SQLException {
        ...
        checkExecutorClosed();
        Status status = new Status();
        com._4paradigm.openmldb.QueryFuture queryFuture = router.CallSQLBatchRequestProcedure(db, spName,
                unit.toMillis(timeOut), currentRowBatch, status);
